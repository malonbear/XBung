...
;(function($, undefined) {
	window.xbung.common.fn = {
		...
		//显示对话框
		showReveal: function(revealID, hideCallbackFn) {
			// var revealID = event.data.revealID,
			var	zIndexMax = -1,
				zIndex;
			
			//隐藏对话框
			var _hideReveal = function() {
				$('#' + revealID)
					.css('z-index', -1)
					.hide();
					
				//执行隐藏时的回调函数
				if(typeof(hideCallbackFn) != 'undefined') {
					hideCallbackFn();
				};
				
				//取消隐藏事件
				$('#' + revealID + '-revealBGID').off('touchend', _hideReveal).remove();
			};
			...
			//最大zindex；
			zIndex = (zIndexMax === -1)? 100: zIndexMax;
				
			$('#' + revealID)
				.css('z-index', zIndex + 1)
				.show();
			// //执行回调函数
			// if(typeof(event.data.showContentFn) !== undefined) {
				// event.data.showContentFn();
			// };
			
			//背景显示，同时关联关闭事件
			$('<div id="' + revealID + '-revealBGID"' + ' class="revealBG-xb"></div>')
				.appendTo('body')
				.css('z-index', zIndex)
				.on('touchend', _hideReveal);
			//$('#' + revealID + '-revealBGID').on('touchend', _hideReveal);
			//$('.revealBG-xb').show();
		},
		
		//隐藏对话框
		hideReveal: function(event) {
			var revealID = event.data.revealID;
			$('#' + revealID).hide();
			$('.revealBG-xb').hide();
		}
